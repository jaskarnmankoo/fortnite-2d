...
class Rectangle extends Shape {
	...
	getRectCorner() {
		return new Pair(
			this.offsetPosition.x - this.width / 2,
			this.offsetPosition.y - this.height / 2
		)
	}
	getGradient(context) {
		var rectCorner = this.getRectCorner()
		var right =
			rectCorner.x + (this.contextStyle.horizontalGrd ? this.width : 0)
		var bottom =
			rectCorner.y + (!this.contextStyle.horizontalGrd ? this.height : 0)

		var x1 = this.contextStyle.autoCalculateGrd
			? rectCorner.x
			: this.contextStyle.x1
		var y1 = this.contextStyle.autoCalculateGrd
			? rectCorner.y
			: this.contextStyle.y1
	...
	getCollisionPoints(rootPosition) {
		...
		collisionPoints.push(
			new Pair(
				this.offsetPosition.x + rootPosition.x - this.width / 2,
				this.offsetPosition.y + rootPosition.y - this.height / 2
			)
		)
		// Bottom-Right Corner
		collisionPoints.push(
...
class Arc extends Shape {
	...
	getRectCorner() {
		return new Pair(
			this.offsetPosition.x - this.radius,
			this.offsetPosition.y - this.radius
		)
	}
	getGradient(context) {
		var rectCorner = this.getRectCorner()
		var right =
			rectCorner.x + (this.contextStyle.horizontalGrd ? 2 * this.radius : 0)
		var bottom =
			rectCorner.y + (!this.contextStyle.horizontalGrd ? 2 * this.radius : 0)
		...
		var r2 = this.contextStyle.autoCalculateGrd
			? this.radius
			: this.contextStyle.r2
	...
	getCollisionPoints(rootPosition) {
		...
		collisionPoints.push(
			new Pair(
				this.offsetPosition.x + rootPosition.x - this.radius,
				this.offsetPosition.y + rootPosition.y - this.radius
			)
		)
		// Bottom-Right
		collisionPoints.push(
...
class Polygon extends Shape {
	...
	getRectCorner() {
		return new Pair(
			this.offsetPosition.x - this.getWidth() / 2,
			this.offsetPosition.y - this.getHeight() / 2
		)
	}
	getGradient(context) {
		var rectCorner = this.getRectCorner()
	...
	draw(context, rootPosition, angle) {
		...
		context.beginPath()
		context.moveTo(
			this.vertices[0].x + this.offsetPosition.x,
			this.vertices[0].y + this.offsetPosition.y
		)
		for (var i = 1; i < this.vertices.length; i++) {
			context.lineTo(
				this.vertices[i].x + this.offsetPosition.x,
				this.vertices[i].y + this.offsetPosition.y
			)
		}
		context.closePath()
	...
	getCollisionPoints(rootPosition) {
		...
		collisionPoints.push(
			new Pair(
				this.offsetPosition.x + rootPosition.x - width / 2,
				this.offsetPosition.y + rootPosition.y - height / 2
			)
		)
		// Bottom-Right
		collisionPoints.push(
...
class Actor {
	...
	fixVelocity() {
		...
		if (x > this.maxVelocity.x) {
			this.velocity.x =
				this.velocity.x > 0 ? this.maxVelocity.x : -this.maxVelocity.x
		}
		if (y > this.maxVelocity.y) {
			this.velocity.y =
				this.velocity.y > 0 ? this.maxVelocity.y : -this.maxVelocity.y
		}
	}
	intPosition() {
		this.x = Math.round(this.position.x)
	...
	getTruePosition() {
		var rootPosition =
			this.parentActor == null
				? this.position
				: this.parentActor.getTruePosition()
		var rootAngle =
			this.parentActor == null ? 0 : this.parentActor.getTrueRotation()
		var vector = this.parentActor == null ? new Pair(0, 0) : this.position
		var rotatedVector = vector.rotateOnAngle(rootAngle + this.rotationAngle)
		return new Pair(
			rootPosition.x + rotatedVector.x,
			rootPosition.y + rotatedVector.y
		)
	}
	...
	getTrueRotation() {
		return this.parentActor == null
			? this.rotationAngle
			: this.rotationAngle + this.parentActor.getTrueRotation()
	}
	...
	isAncestor(actor) {
		if (this.parentActor) {
			if (this.parentActor == actor) {
				return true
			}
			return this.parentActor.isAncestor(actor)
		}
		return false
	}
	// Returns true if the given actor is the same as this one, is a descendant, or is an ancestor
	isInFamilyTree(actor) {
		return this == actor || this.isDescendant(actor) || this.isAncestor(actor)
	}
	// destroy self from the game view
	destroySelf() {
		// Remove all sub-actors
		for (var i = 0; i < this.subActors.length; i++) {
			this.stage.removeActor(this.subActors[i])
		}
	...
	step() {
		...
		if (x != 0 || y != 0) {
			...
			for (var i = 0; i < highestMovement * scaleFactor; i++) {
				...
				var collidingActors = this.stage.getCollidingActors(
					startingPosition.plus(new Pair(nextX, nextY)),
					this
				)
...
class ActorCharacter extends ActorHP {
	...
	getAllyActors() {
		var actors = this.stage.getAllCharacters()
		var allies = []
...
class Player extends ActorCharacter {
	constructor(stage, position, name, shape, speed) {
		super(stage, position, name, shape, speed, 100)
		this.passThrough = false
		this.team = "player"
...
class Bullet extends Actor {
	...
	runCollisionCheck() {
		...
		for (var i = 0; i < collidingActors.length; i++) {
			...
			if (
				!collidingActors[i].canPassThrough(this) &&
...
class Weapon extends Actor {
	constructor(weaponID, stage, position, name, shape) {
		...
		this.owner = null // the owner of the weapon
	...
	shoot(toPosition, shooter) {
		...
		if (this.hasEnoughToFire()) {
			...
			if (
				convertGameTimeToSeconds(_gameTime - this.lastShotTime) >= this.cooldown
			) {
				...
				for (var i = 0; i < this.bulletSpawns.length; i++) {
					...
					for (var j = 0; j < spawn.bulletAmount; j++) {
						...
						var bullet = new Bullet(
							this.stage,
							position,
							this.bulletSpeed * spawn.speedMod,
							this,
							spawn,
							shooter
						)
	...
	spawnDrop() {
		if (this.owner) {
			var weaponDrop = new WeaponPickup(stage, this.owner.position, this)
			this.stage.addActor(weaponDrop)
		}
	}
	toString() {
		return "~" + this.name + "~ : " + this.position.toString()
...
class EnemyActor extends ActorCharacter {
	...
	step() {
		...
		if (this.target) {
			var distance = this.stage.getDistance(this, this.target)
			// shoot if possible
...
class BuildingBlock_HP extends BuildingBlock {
	...
	getBuildPos() {
		var rotatedVector = this.position.rotateOnAngle(this.owner.rotationAngle)
		var realPos = this.owner.getTruePosition().plus(rotatedVector)
		return realPos
	}
	spawnBlock(creator) {
		...
		var copyShape = new Rectangle(
			this.blockShape.width,
			this.blockShape.height,
			style
		)
		var block = new ActorHPBlock(
			this.stage,
			this.getBuildPos(),
			copyShape,
			40 + this.level * 10
...
class StageActor extends Actor {
	constructor(stage, position, name, shapes) {
...
class TransporterActor extends Actor {
	constructor(stage, position) {
		...
		this.toTransporter = null // If this is set, this is the transporter that the actor will be sent to
	}
	setToLocation(position) {
		this.toLocation = position
	}
	setToTransporter(actor) {
		this.toTransporter = actor
	}
	transport(actor) {
		if (this.toTransporter) {
			var position = this.toTransporter.getTruePosition()
...
class Animation {
	constructor() {
		this.time = 0 // time in seconds for this animation
		this.repeats = 1 // how many times this animation repeats

		this.startTime = 0

		// fade
		this.isFading = false
		this.fadeFromStyle = null
		this.fadeToStyle = null
		this.fadeToAlpha = 0
		this.fadeOrigAlpha = 0
