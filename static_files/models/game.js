...
class Stage {
	constructor(canvas) {
		...
		var w = this.createWeapon_Gun(this.player.shapes[0].contextStyle.clone())
		this.player.addWeapon(w)
		this.player.addWeapon(
			this.createWeapon_Rifle(this.player.shapes[0].contextStyle.clone())
		)
		this.player.addWeapon(
			this.createWeapon_SMG(this.player.shapes[0].contextStyle.clone())
		)
		this.player.addWeapon(
			this.createWeapon_Sniper(this.player.shapes[0].contextStyle.clone())
		)
		this.player.addWeapon(
			this.createWeapon_Shotgun(this.player.shapes[0].contextStyle.clone())
		)

		this.player.addBlock(new BuildingBlock_HP(this, 32, 32, 150))
		...
		this.addHealthPickup(
			new Pair(startingPos.x + 300, startingPos.y),
			50,
			false
		)
		this.addHealthPickup(
			new Pair(startingPos.x + 300, startingPos.y + 300),
			50,
			true
		)
	...
	centerCameraOnPlayer() {
		if (this.player) {
			this.centerCameraOnActor(this.player)
			//this.positionOffset = this.getFocusedCameraCoords(this.player.position);
		} else {
			var context = this.canvas.getContext("2d")
			context.restore()
			context.save()
	...
	createWeapon_Gun(style) {
		...
		weapon.addBulletSpawn(
			new BulletSpawn(new Pair(0, 0), 1, 1, 0, new NullShape(), [bulletShape])
		)
		return weapon
	}
	...
	createWeapon_DualPistol(style) {
		...
		var bulletShape = this.createBulletShape(4, 6, new ColourCStyle(0, 0, 0, 1))
		weapon.addBulletSpawn(
			new BulletSpawn(new Pair(6, -3), 1, 1, -ang, new NullShape(), [
				bulletShape,
			])
		)
		weapon.addBulletSpawn(
	...
	createWeapon_Shotgun(style) {
		...
		var bulletShape = this.createBulletShape(4, 6, new ColourCStyle(0, 0, 0, 1))
		weapon.addBulletSpawn(
			new BulletSpawn(new Pair(0, -16), 1, 6, 0, new NullShape(), [bulletShape])
		)
		return weapon
	}
	...
	createWeapon_DualSMG(style) {
		...
		var bulletShape = this.createBulletShape(4, 6, new ColourCStyle(0, 0, 0, 1))
		weapon.addBulletSpawn(
			new BulletSpawn(new Pair(2, -4), 1, 1, -ang, new NullShape(), [
				bulletShape,
			])
		)
		weapon.addBulletSpawn(
	...
	createWeapon_Sniper(style) {
		...
		var bulletShape = this.createBulletShape(
			4,
			12,
			new ColourCStyle(0, 0, 0, 1)
		)
		weapon.addBulletSpawn(
			new BulletSpawn(new Pair(0, -36), 1, 1, 0, new NullShape(), [bulletShape])
		)
		return weapon
	}
	...
	getCollidingActors(position, actor) {
		...
		for (var i = 0; i < allActors.length; i++) {
			if (
				...
				var collision = this.willActorsCollide(
					position,
					actor,
					otherPosition,
					allActors[i]
	...
	KOEnemy(enemyActor) {
		...
		if (Math.random() < enemyActor.healthDropRate) {
			...
			this.addHealthPickup(
				new Pair(
					enemyActor.position.x + randint(25),
					enemyActor.position.y + randint(25)
				),
				20 + this.wave * 5,
				maxHP
			)
	...
	setStage(stageName) {
		if (stageName == "Basic") {
			...
			this.stageActor = new StageActor(
				this,
			...
			this.playerSpawnLocation = new Pair(
				Math.floor(this.width / 2),
				Math.floor(this.height / 2)
			)
			this.enemySpawnLocations = [
				new Pair(
					this.playerSpawnLocation.x + 600,
					this.playerSpawnLocation.y + 600
				),
				new Pair(
					this.playerSpawnLocation.x + 600,
					this.playerSpawnLocation.y - 600
			...
			var fastTerrain = this.createTerrain(
				new Pair(
					this.playerSpawnLocation.x + 400,
					this.playerSpawnLocation.y + 400
				),
				200,
				100,
				2,
			...
			// Transporters
			var t1 = this.createTeleporter(
				new Pair(this.playerSpawnLocation.x + 600, this.playerSpawnLocation.y),
				null
			)
			var t2 = this.createTeleporter(
				new Pair(this.playerSpawnLocation.x - 600, this.playerSpawnLocation.y),
				null
			)
			var t3 = this.createTeleporter(
				new Pair(this.playerSpawnLocation.x, this.playerSpawnLocation.y - 600),
				new Pair(this.playerSpawnLocation.x, this.playerSpawnLocation.y + 600)
			)
