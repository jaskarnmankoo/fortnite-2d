...
app.get(
	"/api/information/user/:username/password/:password",
	function (req, res) {
		var username = req.params.username
		var password = req.params.password
		let sql = "SELECT * FROM player WHERE username = ?;"
...
app.get(
	"/api/verifyPassword/user/:username/pass/:password",
	function (req, res) {
		var username = req.params.username
		var password = req.params.password
		let sql = "SELECT * FROM player WHERE username = ?;"
...
app.post("/api/register/", function (req, res) {
	...
	var score = 1
	let sql =
		"INSERT INTO player(username, password, email, birthday, favColor, year, lecture, score) VALUES (?,?,?,?,?,?,?,?);"

	bcrypt.hash(password, saltRounds, function (err, hash) {
		if (hash) {
			db.run(
				sql,
				[username, hash, email, birthday, favColor, year, lecture, score],
				function (err) {
					var result = {}
					if (err) {
						res.status(409)
...
app.put("/api/update/user/:username", function (req, res) {
	...
	var lecture = req.body.lecture
	let sql =
		"UPDATE player SET password=?, email=?, birthday=?, favColor=?, year=?, lecture=? WHERE username=?"

	bcrypt.hash(password, saltRounds, function (err, hash) {
		if (hash) {
			db.run(
				sql,
				[hash, email, birthday, favColor, year, lecture, username],
				function (err) {
					var result = {}
					if (err) {
						res.status(404)
